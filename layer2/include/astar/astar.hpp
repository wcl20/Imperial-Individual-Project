...
struct AStar {
        std::vector<Node> search(const Node& start, const Node& goal, std::function<bool(int, int, int, int)> colliding, int Nx, int Ny)
        {
                ...
                while (open_set.size() > 0) {
                        std::sort(open_set.begin(), open_set.end(), [&f_score](const Node& a, const Node& b) {
                                        auto it1 = f_score.find(a); auto it2 = f_score.find(b); if(it2 == f_score.end()) return true; if(it1 == f_score.end()) return false; return (f_score[a]<f_score[b]);
                                });
