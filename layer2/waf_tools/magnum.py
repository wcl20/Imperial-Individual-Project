...
def options(opt):
    opt.add_option('--magnum_install_dir', type='string',
                   help='path to magnum install directory', dest='magnum_install_dir')


def get_magnum_components():
    magnum_components = ['Audio', 'DebugTools', 'MeshTools', 'Primitives', 'SceneGraph', 'Shaders', 'Shapes', 'Text', 'TextureTools', 'Trade', 'GlfwApplication', 'GlutApplication', 'GlxApplication', 'Sdl2Application', 'XEglApplication', 'WindowlessCglApplication', 'WindowlessEglApplication', 'WindowlessGlxApplication', 'WindowlessIosApplication',
                         'WindowlessWglApplication', 'WindowlessWindowsEglApplicatio', 'CglContext', 'EglContext', 'GlxContext', 'WglContext', 'OpenGLTester', 'MagnumFont', 'MagnumFontConverter', 'ObjImporter', 'TgaImageConverter', 'TgaImporter', 'WavAudioImporter', 'distancefieldconverter', 'fontconverter', 'imageconverter', 'info', 'al-info']
    magnum_dependencies = {}
    ...
    magnum_dependencies['DebugTools'] = [
        'MeshTools', 'Primitives', 'SceneGraph', 'Shaders']
    ...
    magnum_dependencies['MagnumFontConverter'] = [
        'TgaImageConverter', 'Text', 'TextureTools']
    ...
    pat_plugin = re.compile(
        '^(MagnumFont|MagnumFontConverter|ObjImporter|TgaImageConverter|TgaImporter|WavAudioImporter)$')
    pat_bin = re.compile(
...
def get_magnum_dependency_libs(bld, components, magnum_var='Magnum', corrade_var='Corrade'):
    ...
    requested_components = list(
        set(components.split()).intersection(magnum_components))
    # add dependencies
...
def check_magnum(conf, *k, **kw):
    ...
    libs_check = ['/usr/lib', '/usr/local/lib', '/opt/local/lib',
                  '/sw/lib', '/lib', '/usr/lib/x86_64-linux-gnu/', '/usr/lib64']
    bins_check = ['/usr/bin', '/usr/local/bin',
                  '/opt/local/bin', '/sw/bin', '/bin']
    if conf.env['DEST_OS'] == 'darwin':
        includes_check = includes_check + ['/System/Library/Frameworks/OpenGL.framework/Headers',
        ...
        libs_check = libs_check + ['/System/Library/Frameworks/OpenGL.framework/Libraries',
                                   '/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/']
    ...
    if conf.options.magnum_install_dir:
        includes_check = [conf.options.magnum_install_dir +
                          '/include'] + includes_check
    ...
    magnum_possible_configs = ["BUILD_DEPRECATED", "BUILD_STATIC", "BUILD_MULTITHREADED", "TARGET_GL",
                               "TARGET_GLES", "TARGET_GLES2", "TARGET_GLES3", "TARGET_DESKTOP_GLES", "TARGET_WEBGL", "TARGET_HEADLESS"]
    magnum_config = []
    ...
    try:
        ...
        magnum_includes = magnum_includes + \
            [magnum_include_dir, magnum_include_dir + '/MagnumExternal/OpenGL']
        ...
        # only check for components that can exist
        requested_components = list(
            set(requested_components).intersection(magnum_components))
        # add dependencies
        for lib in requested_components:
            requested_components = requested_components + \
                magnum_dependencies[lib]
        # remove duplicates
        requested_components = list(set(requested_components))
        ...
        for component in requested_components:
            ...
            if component_type == 'lib':
                ...
                include_dir = get_directory(
                    'Magnum/' + include_prefix + '/' + component_file + '.h', includes_check)
                lib = 'Magnum' + component
                lib_dir = get_directory(
                    'lib' + lib + '.' + lib_type, libs_check)

                magnum_component_includes[component] = magnum_component_includes[component] + [
                    include_dir]
                magnum_component_libpaths[component] = magnum_component_libpaths[component] + [
                    lib_dir]
                ...
                if re.match(pat_app, component):
                    if component == 'GlfwApplication':
                        ...
                        glfw_inc = get_directory(
                            'GLFW/glfw3.h', includes_check)
                        ...
                        for lib_glfw in libs_glfw:
                            try:
                                lib_dir = get_directory(
                                    'lib' + lib_glfw + '.' + suffix, libs_check)
                                glfw_found = True

                                magnum_component_libpaths[component] = magnum_component_libpaths[component] + [
                                    lib_dir]
                                magnum_component_libs[component].append(
                                    lib_glfw)
                                break
                            except:
                                glfw_found = False
                        ...
                        glut_inc = get_directory(
                            'GL/freeglut.h', includes_check)
                        ...
                        for lib_glut in libs_glut:
                            try:
                                lib_dir = get_directory(
                                    'lib' + lib_glut + '.' + suffix, libs_check)
                                glut_found = True
                        ...
                        conf.check_cfg(
                            path='sdl2-config', args='--cflags --libs', package='', uselib_store='MAGNUM_SDL')

                        magnum_component_includes[component] = magnum_component_includes[component] + \
                            conf.env['INCLUDES_MAGNUM_SDL']
                        magnum_component_libpaths[component] = magnum_component_libpaths[component] + \
                            conf.env['LIBPATH_MAGNUM_SDL']
                        magnum_component_libs[component] = magnum_component_libs[component] + \
                            conf.env['LIB_MAGNUM_SDL']
                ...
                if component == 'Audio':
                    ...
                    for inc in includes_audio:
                        try:
                            ...
                            incl_audio = get_directory(
                                inc + '/al.h', includes_check, True)
                            openal_found = True
                    ...
                    for lib_audio in libs_audio:
                        try:
                            lib_dir = get_directory(
                                'lib' + lib_audio + '.' + suffix, libs_check)
                            openal_found = True
                ...
                if re.match(pat_audio, component):
                    ...
                    component_file = component.replace(
                ...
                include_dir = get_directory(
                    'MagnumPlugins/' + component + '/' + component_file + '.h', includes_check)
                lib = component
                ...
                lib_dir = get_directory(
                    'magnum/' + lib_path_suffix + lib + '.' + suffix, libs_check, True)
                ...
                magnum_component_bins[component] = magnum_component_bins[component] + [
                    executable]
        ...
        if conf.env['DEST_OS'] == 'darwin':
            conf.env['FRAMEWORK_%s_Magnum' %
                     magnum_var] = ['OpenGL', 'Foundation']
        ...
        conf.env['DEFINES_%s' % magnum_var].append(
            '%s_PLUGINS_DIR="%s"' % (magnum_var.upper(), magnum_plugins_dir))
        conf.env['DEFINES_%s' % magnum_var].append(
            '%s_PLUGINS_FONT_DIR="%s"' % (magnum_var.upper(), magnum_plugins_font_dir))
        conf.env['DEFINES_%s' % magnum_var].append('%s_PLUGINS_FONTCONVERTER_DIR="%s"' % (
            magnum_var.upper(), magnum_plugins_fontconverter_dir))
        conf.env['DEFINES_%s' % magnum_var].append('%s_PLUGINS_IMAGECONVERTER_DIR="%s"' % (
            magnum_var.upper(), magnum_plugins_imageconverter_dir))
        conf.env['DEFINES_%s' % magnum_var].append('%s_PLUGINS_IMPORTER_DIR="%s"' % (
            magnum_var.upper(), magnum_plugins_importer_dir))
        conf.env['DEFINES_%s' % magnum_var].append('%s_PLUGINS_AUDIOIMPORTER_DIR="%s"' % (
            magnum_var.upper(), magnum_plugins_audioimporter_dir))
        for config in magnum_config:
            conf.env['DEFINES_%s' % magnum_var].append(config)
        if conf.env['DEST_OS'] == 'darwin':
            conf.env['DEFINES_%s' % magnum_var].append(
                '%s_MAC_OSX' % magnum_var.upper())

        # copy C++ defines to Magnum::Magnum component; we want them to be available on all Magnum builds
        conf.env['DEFINES_%s_Magnum' % magnum_var] = copy.deepcopy(
            conf.env['DEFINES_%s' % magnum_var])

        # set component libs
        for component in requested_components:
            conf.env['INCLUDES_%s_%s' % (
                magnum_var, component)] = magnum_component_includes[component]
            conf.env['LIBPATH_%s_%s' % (
                magnum_var, component)] = magnum_component_libpaths[component]
            conf.env['LIB_%s_%s' %
                     (magnum_var, component)] = magnum_component_libs[component]
            conf.env['EXEC_%s_%s' %
                     (magnum_var, component)] = magnum_component_bins[component]
    except:
        if required:
