...
def options(opt):
    opt.add_option('--magnum_integration_install_dir', type='string',
                   help='path to magnum plugins install directory', dest='magnum_integration_install_dir')
...
def get_magnum_integration_components():
    ...
    for component in magnum_integration_components:
        ...
        if component == 'Bullet':
            magnum_integration_magnum_dependencies[component] += [
                'SceneGraph', 'Shaders']
        elif component == 'Dart':
            magnum_integration_magnum_dependencies[component] += [
                'SceneGraph', 'Primitives', 'MeshTools']
...
def get_magnum_integration_dependency_libs(bld, components, magnum_integration_var='MagnumIntegration', magnum_var='Magnum', corrade_var='Corrade'):
    ...
    requested_components = list(
        set(components.split()).intersection(magnum_integration_components))
    # add dependencies
    for lib in requested_components:
        requested_components = requested_components + \
            magnum_integration_dependencies[lib]
    # remove duplicates
    ...
    sorted_components = [magnum_integration_var +
                         '_' + c for c in sorted_components]
...
def check_magnum_integration(conf, *k, **kw):
    ...
    if conf.options.magnum_integration_install_dir:
        includes_check = [
            conf.options.magnum_integration_install_dir + '/include'] + includes_check
        libs_check = [
            conf.options.magnum_integration_install_dir + '/lib'] + libs_check
        bins_check = [
            conf.options.magnum_integration_install_dir + '/bin'] + bins_check
    ...
    # only check for components that can exist
    requested_components = list(
        set(requested_components).intersection(magnum_integration_components))
    # add dependencies
    for lib in requested_components:
        requested_components = requested_components + \
            magnum_integration_dependencies[lib]
    ...
    for component in requested_components:
        ...
        try:
            include_dir = get_directory(
                'Magnum/' + component_name + '/' + component_file + '.h', includes_check)
            magnum_integration_includes = magnum_integration_includes + \
                [include_dir]

            magnum_integration_component_includes[component] = magnum_integration_component_includes[component] + [
                include_dir]
            if component != 'Eigen':
                lib = 'Magnum' + component_name
                lib_dir = get_directory(
                    'lib' + lib + '.' + suffix, libs_check, True)
                magnum_integration_libs.append(lib)
                magnum_integration_libpaths = magnum_integration_libpaths + \
                    [lib_dir]

                magnum_integration_component_libpaths[component] = magnum_integration_component_libpaths[component] + [
                    lib_dir]
    ...
    conf.env['INCLUDES_%s' %
             magnum_integration_var] = magnum_integration_includes
    conf.env['LIBPATH_%s' %
             magnum_integration_var] = magnum_integration_libpaths
    conf.env['LIB_%s' % magnum_integration_var] = magnum_integration_libs
    conf.env['DEFINES_%s' % magnum_integration_var] = copy.deepcopy(
    ...
    for component in requested_components:
        for lib in magnum_integration_dependencies[component]:
            magnum_integration_component_includes[component] = magnum_integration_component_includes[component] + \
                magnum_integration_component_includes[lib]
            magnum_integration_component_libpaths[component] = magnum_integration_component_libpaths[component] + \
                magnum_integration_component_libpaths[lib]
            magnum_integration_component_libs[component] = magnum_integration_component_libs[component] + \
                magnum_integration_component_libs[lib]

        conf.env['INCLUDES_%s_%s' % (magnum_integration_var, component)] = list(
            set(magnum_integration_component_includes[component]))
        if len(magnum_integration_component_libs[component]) > 0:
            conf.env['LIBPATH_%s_%s' % (magnum_integration_var, component)] = list(
                set(magnum_integration_component_libpaths[component]))
            conf.env['LIB_%s_%s' % (magnum_integration_var, component)] = list(
                set(magnum_integration_component_libs[component]))

        # copy the C++ defines; we want them to be available on all Magnum builds
        conf.env['DEFINES_%s_%s' % (magnum_integration_var, component)] = copy.deepcopy(
            conf.env['DEFINES_%s' % magnum_integration_var])
    # set C++ flags
    conf.env['CXX_FLAGS_%s' % magnum_integration_var] = copy.deepcopy(
        conf.env['CXX_FLAGS_%s' % magnum_var])
