...
def options(opt):
    opt.add_option('--magnum_plugins_install_dir', type='string',
                   help='path to magnum plugins install directory', dest='magnum_plugins_install_dir')


def get_magnum_plugins_components():
    magnum_plugins_components = ['AnyAudioImporter', 'AnyImageConverter', 'AnyImageImporter', 'AnySceneImporter', 'AssimpImporter', 'DdsImporter', 'DevIlImageImporter', 'DrFlacAudioImporter', 'DrWavAudioImporter', 'FreeTypeFont',
                                 'HarfBuzzFont', 'JpegImporter', 'MiniExrImageConverter', 'OpenGexImporter', 'PngImageConverter', 'PngImporter', 'StanfordImporter', 'StbImageConverter', 'StbImageImporter', 'StbTrueTypeFont', 'StbVorbisAudioImporter']
...
def get_magnum_plugins_dependency_libs(bld, components, magnum_plugins_var='MagnumPlugins', magnum_var='Magnum', corrade_var='Corrade'):
    ...
    requested_components = list(
        set(components.split()).intersection(magnum_plugins_components))
    # add dependencies
    for lib in requested_components:
        requested_components = requested_components + \
            magnum_plugins_dependencies[lib]
    # remove duplicates
    ...
    sorted_components = [magnum_plugins_var +
...
def check_magnum_plugins(conf, *k, **kw):
    ...
    if conf.options.magnum_plugins_install_dir:
        includes_check = [
            conf.options.magnum_plugins_install_dir + '/include'] + includes_check
        libs_check = [
            conf.options.magnum_plugins_install_dir + '/lib'] + libs_check
        bins_check = [
            conf.options.magnum_plugins_install_dir + '/bin'] + bins_check
    ...
    # only check for components that can exist
    requested_components = list(
        set(requested_components).intersection(magnum_plugins_components))
    # add dependencies
    for lib in requested_components:
        requested_components = requested_components + \
            magnum_plugins_dependencies[lib]
    ...
    for component in requested_components:
        ...
        try:
            ...
            lib_dir = get_directory(
                'magnum/' + lib_path_suffix + lib + '.' + modules_suffix, libs_check, True)
            # magnum_plugins_libs.append(lib)
            # magnum_plugins_libpaths = magnum_plugins_libpaths + [lib_dir]

            magnum_plugins_component_includes[component] = magnum_plugins_component_includes[component] + [
                include_dir]
        ...
        if component == 'AssimpImporter':
            ...
            try:
                ...
                magnum_plugins_includes = magnum_plugins_includes + \
                    [assimp_inc]
                magnum_plugins_component_includes[component] = magnum_plugins_component_includes[component] + [
                    assimp_inc]
                ...
                magnum_plugins_component_libpaths[component] = magnum_plugins_component_libpaths[component] + [
                    lib_dir]
            ...
            try:
                freetype_inc = get_directory(
                    'freetype/ft2build.h', includes_check, True)
    ...
    conf.env['DEFINES_%s' % magnum_plugins_var] = copy.deepcopy(
    ...
    for component in requested_components:
        ...
        conf.env['INCLUDES_%s_%s' % (magnum_plugins_var, component)] = list(
            set(magnum_plugins_component_includes[component]))
        conf.env['LIBPATH_%s_%s' % (magnum_plugins_var, component)] = list(
            set(magnum_plugins_component_libpaths[component]))
        conf.env['LIB_%s_%s' % (magnum_plugins_var, component)] = list(
            set(magnum_plugins_component_libs[component]))

        # copy the C++ defines; we want them to be available on all Magnum builds
        conf.env['DEFINES_%s_%s' % (magnum_plugins_var, component)] = copy.deepcopy(
            conf.env['DEFINES_%s' % magnum_plugins_var])
    # set C++ flags
    conf.env['CXX_FLAGS_%s' % magnum_plugins_var] = copy.deepcopy(
