...
def options(opt):
    opt.add_option('--corrade_install_dir', type='string',
                   help='path to corrade install directory', dest='corrade_install_dir')
...
def check_corrade(conf, *k, **kw):
    ...
    corrade_possible_configs = ["GCC47_COMPATIBILITY", "MSVC2015_COMPATIBILITY", "MSVC2017_COMPATIBILITY", "BUILD_DEPRECATED", "BUILD_STATIC", "TARGET_UNIX", "TARGET_APPLE",
    ...
    corrade_components = ['Containers', 'PluginManager',
    ...
    try:
        ...
        corrade_include_dir = get_directory(
            'Corrade/Corrade.h', includes_check)
        ...
        requested_components = list(
            set(requested_components).intersection(corrade_components))
        # add dependencies
        for lib in requested_components:
            requested_components = requested_components + \
                corrade_dependencies[lib]
        # remove duplicates
        ...
        for component in requested_components:
            ...
            corrade_component_includes[component] = [
                copy.deepcopy(corrade_include_dir)]
            # get component type
            component_type = corrade_component_type[component]
            if component_type == 'include':
                include_dir = get_directory(
                    'Corrade/' + component + '/' + component + '.h', includes_check)
                corrade_includes = corrade_includes + [include_dir]
                corrade_component_includes[component] = corrade_component_includes[component] + [
                    include_dir]
                ...
                corrade_component_libpaths[component] = corrade_component_libpaths[component] + [
                    lib_dir]
                ...
                corrade_component_bins[component] = corrade_component_bins[component] + [
                    executable]
        # remove duplicates
        ...
        for component in requested_components:
            for lib in corrade_dependencies[component]:
                corrade_component_includes[component] = corrade_component_includes[component] + \
                    corrade_component_includes[lib]
                corrade_component_libpaths[component] = corrade_component_libpaths[component] + \
                    corrade_component_libpaths[lib]
                corrade_component_libs[component] = corrade_component_libs[component] + \
                    corrade_component_libs[lib]
                corrade_component_bins[component] = corrade_component_bins[component] + \
                    corrade_component_bins[lib]

            conf.env['INCLUDES_%s_%s' % (corrade_var, component)] = list(
                set(corrade_component_includes[component]))
            conf.env['LIBPATH_%s_%s' % (corrade_var, component)] = list(
                set(corrade_component_libpaths[component]))
            conf.env['LIB_%s_%s' % (corrade_var, component)] = list(
                set(corrade_component_libs[component]))
            conf.env['EXEC_%s_%s' % (corrade_var, component)] = list(
                set(corrade_component_bins[component]))
        # set C++ flags
...
def corrade_enable_pedantic_flags(conf, corrade_var='Corrade'):
    ...
    conf.env['CXXFLAGS'] = list(
        set(conf.env['CXXFLAGS'] + conf.env['CXX_FLAGS_%s' % corrade_var]))
...
class readFile(Task):
    ...
    def scan(self):
        ...
        config_file_path = config_file.abspath(
        )[:config_file.abspath().rfind('/')]
        try:
        ...
        for line in config_content:
            ...
            if results:
                ...
                dependencies.append(
                    self.generator.bld.root.find_node(filename))
...
def corrade_add_resource(bld, name, config_file, corrade_var='Corrade'):
    ...
    corrade_bin = (" ".join(
        s for s in bld.env['EXEC_%s_rc' % corrade_var] if 'corrade-rc' in s)).split()[0]
    ...
    bld(rule=corrade_bin + ' ' + name + ' ' + full_config_path + '/' +
        short_config + ' ${TGT}', source=read1.outputs, target=target_resource)
...
def corrade_add_plugin(bld, name, config_file, source, use='', uselib='', includes='.', cxxflags='', defines='', corrade_var='Corrade'):
    ...
    plugin_lib = bld.program(features='cxx cxxshlib',
                             source=source,
                             includes=bld.env['INCLUDES_%s_PluginManager' %
                                              corrade_var] + includes.split(),
                             use=use,
                             uselib=uselib,
                             cxxflags=bld.env['CXXFLAGS'] + cxxflags.split(),
    ...
    bld(rule='cp ${SRC} ${TGT}', source=bld.path.make_node(config_file),
        target=bld.path.get_bld().make_node(config_file[config_file.rfind('/') + 1:]))
